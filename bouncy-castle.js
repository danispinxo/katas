...
const totalVolume = function (solids) {
  ...
  for (let i = 0; i < solids.length; i++) { 
    if (solids[i].type === 'sphere') {
      volumeCalc += sphereVolume(solids[i].radius);
    } else if (solids[i].type === 'cone') {
      volumeCalc += coneVolume(solids[i].radius, solids[i].height);
    } else if (solids[i].type === 'prism') {
      volumeCalc += prismVolume(solids[i].height, solids[i].width, solids[i].depth);
    }
  }
  return volumeCalc;
}
